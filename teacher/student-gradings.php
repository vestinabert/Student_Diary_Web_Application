...
if (isset($_SESSION['teacher_id']) && isset($_SESSION['role'])) {
  if ($_SESSION['role'] == 'Teacher') {
     ...
     if (empty($_POST['fname']) || empty($_POST['lname'])){
      ...
      $em  = "Neįvedėte vardą arba pavardę";
      header("Location: gradings.php?error1=$em");
      exit;
     }
?>
<!DOCTYPE html>
...
<body>
    ...
    <a href="gradings.php"><button class="grizti add"> Grįžti </button></a>
      <h3  style = "margin-top: 30px;"> Pažymiai: </h3>
          ...
          <?php
              if (is_array($fetchData)){ $sn = 1;
                foreach($fetchData as $data){
                  if($fname==$data['fname']&&$lname==$data['lname']&& $teacher['subjects']==$data['subject_id']){  
          ?>
          <tr>
            <td><?= $sn; ?></td>
            <td><?= $data['fname']; ?></td>
            ...
            <td><?= $data['grading']; ?></td>
            <td>
              <a href="student-edit.php?student_id=<?=$data['student_id']?>&&grading_id=<?=$data['grading_id']?>&&grading=<?=$data['grading']?>" class="btn btn-warning">Atnaujinti</a>
              <a href="student-delete.php?grading_id=<?=$data['grading_id']?>" class="btn btn-danger">Ištrinti</a>
            </td>
          </tr>
          <?php $sn++; }}} else { ?>
